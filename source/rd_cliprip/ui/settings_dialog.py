...
class SettingsDialog(QDialog):
    ...
    def _build_downloads_tab(self) -> None:
        ...
        behaviour_form.addRow("Simultaneous downloads:", concurrent_row)
        ...
        behaviour_form.addRow("Auto-retry failed downloads:", retry_row)
        behaviour_form.addRow(
            self._hint(
                "How many times a failed download is retried before it is marked "
                "as Failed. Retried items stay in the queue; only a few cases are "
                "never retried automatically (e.g. 404 / video removed / private or "
                "invalid links). Set to 0 to disable and only retry manually."
            )
        )
        ...
        behaviour_form.addRow("Max download speed (per agent):", self.speed_spin)
        ...
        behaviour_form.addRow("Concurrent HLS fragments:", self.fragments_spin)
        behaviour_form.addRow(
            self._hint(
        ...
        behaviour_form.addRow("Downloader:", self.downloader_combo)
        ...
        behaviour_form.addRow("aria2c connections per file:", self.aria2c_connections_spin)
        behaviour_form.addRow(
            self._hint(
                "aria2c can speed up direct MP4 downloads with parallel "
                "connections; install it via Tools > aria2c Settings first. "
                "HLS/DASH streams always use fragment downloads."
            )
        )

        page_layout.addWidget(behaviour_group)
    ...
    def _build_video_tab(self) -> None:
        ...
        self.format_combo.setCurrentIndex({"mp4": 0, "mkv": 1, "webm": 2}.get(fmt, 0))
        ...
        proc_form.addRow(
            self._hint(
                "Off by default: MP4 already prefers H.264/AAC files, which play "
                "and show thumbnails everywhere (MEGA, phones, etc.). Turn this on "
                "only if you want to force other sources into an MP4 container."
            )
        )

        page_layout.addWidget(proc_group)
        page_layout.addStretch()
        self.tabs.addTab(page, "Video")
    ...
    def _build_network_tab(self) -> None:
        ...
        page_layout.addWidget(network_group)
        page_layout.addStretch()
    ...
    def _build_advanced_tab(self) -> None:
        ...
        page_layout.addWidget(cookies_group)
        page_layout.addStretch()
